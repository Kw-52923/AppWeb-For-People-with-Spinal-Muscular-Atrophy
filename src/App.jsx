...
function App() {
  ...
  // --- PANTALLA DE EMERGENCIA GLOBAL (Overlay) ---
  if (emergencyActive) {
    return (
      <div className="fixed inset-0 bg-gradient-to-br from-alert to-red-700 z-[9999] flex flex-col items-center justify-center p-4 sm:p-6 md:p-8 text-center animate-pulse-slow">
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-4 sm:p-6 mb-6 sm:mb-8 max-w-md w-full">
          <div className="space-y-3 text-left">
            ...
            <div className="flex items-center gap-3 text-white">
              ...
              <span className="text-sm sm:text-base">🚑 Servicios de emergencia - Conectando...</span>
            </div>
          </div>
        </div>
        ...
        <button 
          ...
          className="bg-white text-alert px-6 sm:px-10 py-4 sm:py-5 rounded-full font-bold text-base sm:text-lg md:text-xl shadow-2xl hover:scale-105 active:scale-95 transition-transform min-h-touch"
        >
  ...
  // --- SETUP PACIENTE ---
  if (screen === 'patient-setup') {
    return <PatientSetup onComplete={handleSetupComplete} />;
  }

  // --- SETUP CUIDADOR ---
  if (screen === 'caregiver-setup') {
    return <CaregiverSetup onComplete={handleSetupComplete} />;
  }

  // --- RENDERIZADO DE VISTAS ---
  return (
    <div className="min-h-screen bg-gradient-to-br from-bg-gradient-start to-bg-gradient-end pb-safe-area">
      {/* Contenedor responsive - se expande en pantallas grandes */}
      <div className="w-full max-w-7xl mx-auto min-h-screen bg-white md:shadow-2xl relative">
        
        {/* Header Desktop y Móvil - Siempre visible */}
            ...
            <div className="flex justify-between items-center max-w-6xl mx-auto">
              ...
              {!isMobile && (
                <nav className="flex gap-2 items-center">
                  <button 
                    onClick={() => navigate(userRole === 'patient' ? 'home' : 'carehub')}
                    className={`px-4 py-2 rounded-xl transition-all ${(screen === 'home' || screen === 'carehub') ? 'bg-white/20' : 'hover:bg-white/10'}`}
                  >
                  ...
                  <button 
                    ...
                    className={`px-4 py-2 rounded-xl transition-all ${screen === 'medication' ? 'bg-white/20' : 'hover:bg-white/10'}`}
                  >
                  ...
                  <button 
                    ...
                    className={`px-4 py-2 rounded-xl transition-all ${screen === 'community' ? 'bg-white/20' : 'hover:bg-white/10'}`}
                  >
                  ...
                  <button 
                    ...
                    className={`px-4 py-2 rounded-xl transition-all ${screen === 'profile' ? 'bg-white/20' : 'hover:bg-white/10'}`}
                  >
                  ...
                  <button 
                    ...
                    className="px-4 py-2 rounded-xl hover:bg-red-500/20 transition-all"
                  >
        ...
        {/* Navigation Bar Inferior - Full Width (PC + Móvil) */}
        <nav className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-slate-200 px-2 sm:px-4 py-2 sm:py-3 flex justify-around z-40 shadow-2xl">
          <NavIcon 
            active={screen === 'home' || screen === 'carehub'} 
            label="Inicio" 
            icon={Home}
            onClick={() => navigate(userRole === 'patient' ? 'home' : 'carehub')}
          />
          <NavIcon 
            active={screen === 'painmap'} 
            label="Mapa Dolor" 
            icon={Activity}
  ...
  <div className="max-w-4xl mx-auto">
    ...
    <div className="space-y-4">
      <div className="bg-white p-6 rounded-2xl shadow-md border-2 border-growth-green/20">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 bg-growth-green/10 rounded-2xl flex items-center justify-center text-3xl">
              💊
            </div>
            <div>
      ...
      <div className="bg-white p-6 rounded-2xl shadow-md border-2 border-roche-blue/20">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 bg-roche-blue/10 rounded-2xl flex items-center justify-center text-3xl">
              💉
            </div>
            <div>
    ...
    <div className="mt-6">
      <BigButton variant="accent" fullWidth>
        + Añadir Medicamento
      </BigButton>
    </div>
  </div>
